...
10. 新增独立工具 `workloom_task_align`：`action=review` 只返回 snapshot/hash；`action=confirm` 要求 `expectedPrdHash`，校验内容未变化后原子写入凭据；工具只允许主会话调用，不进入 executor 默认工具面。
11. 相同 hash 重复 confirm 为幂等且不刷新 `passedAt`；PRD 结构错误、含占位符、开放节点非 `none` 或 hash 不一致时不得修改 `task.json`。
12. PRD 变化使 alignment stale；后续阶段修改需求时只重开受影响分支并重新确认。
13. `in_progress` 期间 PRD stale 时状态不回退，但以独立 `stale_alignment` gate 阻止 executor 新派发与 continuation，以及 check/archive；重新 alignment 后恢复。
14. start 和 stale 门禁允许 `force + reason` 单次绕过并记录对应 override，但不得生成或更新 alignment 凭据。
...
17. 已归档旧任务保持原样；旧 `in_progress` 任务不追溯阻断；旧 `planning` 任务须重新 alignment。旧文件中的 `grilling` 原样保留但不参与新语义，新任务只写 `alignment`，不建立永久双字段兼容分支。
18. 更新 legacy-module 规范，允许有明确边界、审计说明和测试覆盖的数据模型迁移；禁止静默重解释旧字段。
...
21. Core、assets、workflow contract、DSH adapter、Pi adapter 必须同版本发布。Core 与 workflow 通过 protocol version 握手，DSH apply/Pi 工厂启动时校验；构建测试另行断言四个 package semver 一致。
22. 为新 skill 建立 should-trigger、should-not-trigger 与流程行为 eval，并生成 eval viewer 供用户评审。
...
4. `workloom_task_align` 的 review/confirm 公共接口覆盖成功、换行归一化、全文 hash、hash 冲突、开放节点标记、PRD 非法、重复 confirm 不刷新时间和写入原子性。
5. Start 对新 planning task 强制要求有效 alignment 与合法 PRD；force 绕过只留下 override。
6. PRD 修改后 hash 校验判定 stale；planning 无法 start，in_progress 无法新派发或 continuation executor，也无法进入 check/archive；`stale_alignment` force 逐次留痕，重新确认后恢复。
7. 旧 archived、in_progress、planning 三类任务分别符合既定迁移策略，且无双字段永久兼容分支。
...
10. 全量 lint、typecheck、build、core/adapter 测试通过；protocol 启动校验与四包 semver 一致性均有自动化测试；并完成 DSH headless、Pi 沙箱各一次真实 planning smoke test。
11. 两个 runtime 的 smoke test 均观察到 planning 自动进入统一 alignment，不重复加载 generic `grilling` 或旧 workloom skill，且不改动当前 Web GUI 绑定。
12. Skill eval 覆盖应触发、近似但不应触发、简单任务自然收敛、UI/TDD 分支和需求变更重开分支；eval viewer 完成用户评审。
...
- Stale 使用独立 gate，覆盖 executor 新派发与 continuation；alignment 工具仅由主会话调用。
- 采用 protocol 启动握手、四包 semver 构建检查和同版本原子升级，不为新旧概念模型增加长期兼容复杂度。
- 旧 `grilling` 数据只作为原样保留的历史字段；通过更新 legacy-module 规范为显式迁移建立边界。
- 新 skill 必须经过触发与流程行为 eval，并由用户通过 eval viewer 评审。
