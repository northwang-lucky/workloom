...
import { ERR_PREFIX, executeAlignTask, executeArchiveTask, executeCheckTask, executeCreateTask, executeFinishTask, executeListTasks, executeStartTask, PARAM_DESCRIPTIONS, requireWorkloomCwd, TOOL_DESCRIPTIONS, TOOL_NAMES, } from '@workloom-ai/core';
import { CONTEXT_KEY_PREFIX } from './constants.js';
/**
 * 注册六个任务管理工具（create/start/check/finish/archive/list）。
 * @param ctx 插件作用域上下文
 */
export function registerTaskTools(ctx) {
...
async function startTaskTool(args, exec) {
    const typed = args;
    const cwd = cwdOf(exec);
...
async function checkTaskTool(args, exec) {
    const typed = args;
    const cwd = cwdOf(exec);
 ...
 */
async function alignTaskTool(args, exec) {
    const typed = args;
    const agent = exec.agent;
    if (agent !== undefined) {
        const depth = delegationDepthOf(agent);
        if (depth > 0) {
            throw new Error(`${ERR_PREFIX.taskTool}: workloom_task_align can only be called from the main session ` +
...
async function finishTaskTool(args, exec) {
    const typed = args;
    const cwd = cwdOf(exec);
...
async function archiveTaskTool(args, exec) {
    const typed = args;
    const cwd = cwdOf(exec);
...
async function listTasksTool(args, exec) {
    const typed = args;
    const cwd = cwdOf(exec);
    const [err, result] = await executeListTasks(cwd, typeof typed.status === 'string' ? typed.status : undefined);
    if (err !== null || result === null) {
...
/** 渲染任务工具结果（结构化摘要文本）。 */
function renderTask(_args, value) {
