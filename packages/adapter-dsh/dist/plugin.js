...
import { assembleBreadcrumbSync, assembleSessionContext, assertWorkflowProtocolVersion, composeLocalDirectivesText, findWorkloomRoot, parseContract, WorkflowContractError, } from '@workloom-ai/core';
 ...
 * @returns 注入目标或 null
 */
function resolveInjectionTarget(ctx, context) {
    const agent = context?.agent ?? ctx.agents.currentInitiator();
    if (agent === undefined)
        return null;
 ...
 * @returns 注入文本（可能为空串）
 */
export function renderSessionContext(target) {
    const contractText = loadWorkflowContractText();
    if (contractText === null)
        return '';
    ...
    return assembleSessionContextText(target, contractText, delegationDepthOf(target.agent), localErr !== null ? '' : directives);
}
/**
 ...
 * @returns 注入文本（可能为空串）
 */
export function assembleSessionContextText(target, contractText, delegationDepth = 0, localDirectives = '') {
    const [parseErr, contract] = parseContract(contractText);
    if (parseErr || contract === null) {
        console.warn(`${CONTEXT_WARN_PREFIX} ${parseErr?.message ?? 'contract parse returned no contract'}`);
        return '';
    }
 ...
 */
function extractUserPrompt(agent) {
 ...
 */
function assertLoadedWorkflowProtocol() {
