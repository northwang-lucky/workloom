...
import { ERR_PREFIX as SURFACE_ERR_PREFIX, lookupWorkflowStep, PARAM_DESCRIPTIONS, TOOL_DESCRIPTIONS, TOOL_NAMES, } from '@workloom-ai/core';
...
const LEAF_STEP_HINT = (stepId) => `Workflow step ${stepId} is managed by the main session. ` +
 ...
 */
export function parseSkillFrontmatter(markdownText) {
    try {
 ...
 * @returns 解析结果
 */
function parseSkillFrontmatterInternal(markdownText) {
 ...
 */
export function registerStepsTool(ctx) {
    ctx.tools.register({
 ...
 */
function executeStepTool(args, exec) {
    const params = args;
