 ...
 */
export async function assembleBreadcrumb(params) {
 ...
 * @returns [err, text]：err 为任一编排步骤的失败；skip 命中时 text 为 null
 */
export function assembleBreadcrumbSync(params) {
    try {
        // 深度>0（executor 等叶子子代理）：完全不注入 breadcrumb，跳过全部编排。
        if ((params.delegationDepth ?? 0) > 0)
            return [null, null];
 ...
 */
function assembleBreadcrumbInternal(params) {
    ...
    if (shouldSkipBreadcrumb(config, params.userPrompt ?? ''))
        return null;
 ...
 * @returns 项目根绝对路径
 */
function requireWorkloomRoot(root) {
 ...
 * @returns 合并后的契约
 */
function mergeOverlayOrThrow(contract, overlayText) {
 ...
 * @returns 契约状态
 */
function resolveContractStatus(root, contextKey) {
    const [ptrErr, taskRelPath] = resolveActiveTask(root, contextKey);
    if (ptrErr)
        throw ptrErr;
    if (taskRelPath === null)
        return CONTRACT_STATUS.NO_TASK;
