 ...
 */
export function assembleSessionContext(params) {
    try {
 ...
 * @returns 快照文本
 */
function assembleInternal(params) {
 ...
 * @returns 配置对象或 null
 */
function loadConfigSafely(root) {
    try {
 ...
 * @returns 节行列表（可能为空）
 */
function executorProfilesLines(params, config) {
    if (config === null)
        return [];
    try {
 ...
 * @returns 段行列表（可能为空）
 */
function guidelinesLines(root, config) {
    if (config === null)
        return [];
    const [specErr, spec] = collectSpecIndexes(root, config);
    if (specErr)
        throw specErr;
    if (spec === null || spec.indexes.length === 0)
        return [];
 ...
 * @returns 活跃任务行与任务记录
 */
function activeTaskContext(params) {
    const [ptrErr, taskRelPath] = resolveActiveTask(params.root, params.contextKey);
    if (ptrErr)
        throw ptrErr;
    if (taskRelPath === null)
        return { line: NO_ACTIVE_TASK_LINE, task: null };
 ...
 */
function lastDispatchLines(task) {
    if (task === null)
        return [];
    ...
    if (latest === undefined)
        return [];
    if (typeof latest.kind !== 'string' || latest.kind === '')
        return [];
