 ...
 */
export function applyFixesAndMeasure(root, fixableSnapshot) {
    applyFixes(root);
...
function fixParentChildGaps(root) {
    ...
    for (const name of changed) {
        ...
        if (node !== undefined)
            writeTaskRecord(root, node);
    }
}
/** 修复②：active-pointer 清理（删除指向不存在/归档任务的指针文件）。 */
function fixActivePointers(root) {
    const nodes = collectTasks(root);
    const byName = new Map(nodes.map((node) => [node.name, node]));
...
function fixCompletedArchive(root) {
    const nodes = collectTasks(root);
    for (const node of nodes) {
        ...
        if (node.archived)
            continue;
        if (rec.check === null)
            continue; // 无 check 凭据：拒绝迁移（保留为 manual）
        const now = new Date();
...
function writeTaskRecord(root, node) {
