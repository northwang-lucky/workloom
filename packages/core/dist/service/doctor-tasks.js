...
/** 拉平全部检查的 issue 列表。 */
export function allIssues(checks) {
...
export function pushIssues(issueMap, code, issues) {
    const target = issueMap.get(code);
    if (target !== undefined)
        target.push(...issues);
}
/** 任务级 issue 的 task.json 路径（相对项目根）。 */
export function taskJsonPath(relPath) {
    return join(WORKLOOM_DIR, relPath, 'task.json');
}
/** 规范化父子引用：tasks/<name>。 */
export function canonicalRef(name) {
    return join(TASK_DIR, name);
}
/** 指针文件路径（相对项目根，给 issue.path 用）。 */
export function pointerPath(contextKey) {
...
//# sourceMappingURL=doctor-tasks.js.map