...
import { detectLegacyTrellis, findWorkloomRoot, insideWorkloom, LEGACY_TRELLIS_DIR, WORKLOOM_DIR, } from './locate.js';
/** 错误消息前缀（运行时文案英文）。 */
...
function migrateLegacyTrellisInternal(root, params) {
    ...
    for (const dirName of LEGACY_DIRS) {
        ...
        const copied = copyDirRecursive(sourceDir, targetDir, legacyRoot, projectRoot, skipped, unsupported);
        // migrated 记录「本次实际新写入的区域」：全部冲突跳过的区域不算迁移（幂等语义）；
        // 但首次创建的空目录区域（copied=0 且目标此前不存在）仍算迁移。
        if (copied > 0 || !existedBefore)
...
function copyDirRecursive(sourceDir, targetDir, sourceRoot, projectRoot, skipped, unsupported) {
    ...
    for (const entry of readdirSync(sourceDir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
            copied += copyDirRecursive(sourcePath, targetPath, sourceRoot, projectRoot, skipped, unsupported);
        }
...
function mapKnownFields(doc, droppedConfigFields) {
    ...
    if (doc.max_journal_lines !== undefined) {
        if (typeof doc.max_journal_lines === 'number' &&
...
function collectNonDefault(config) {
    ...
    if (config.contextInjection.maxArtifactBytes !== DEFAULT_CONFIG.contextInjection.maxArtifactBytes) {
