...
export function assertWorkflowProtocolVersion(contractVersion) {
    if (contractVersion !== WORKFLOW_PROTOCOL_VERSION) {
        throw new Error(`${VERSION_MISMATCH_PREFIX}: expected ${WORKFLOW_PROTOCOL_VERSION} but asset carries ${String(contractVersion)} (rebuild and re-sync the assets package with core before activating)`);
    }
}
