...
function profileRowLine(kind, resolved, config) {
    ...
    if (resolved.effort !== undefined)
        parts.push(`effort ${resolved.effort}`);
    if (toolsConfigured)
        parts.push(toolsSummary(/** @type {import('./config.d.ts').SubagentTools} */ (resolved.tools)));
...
function summarizeToolList(items) {
    if (items.length <= TOOL_LIST_KEEP)
        return items.join(', ');
