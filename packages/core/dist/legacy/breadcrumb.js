...
export function mergeOverlay(contract, overlayText) {
    ...
    for (const status of overlay.breadcrumbs.keys()) {
        if (!contract.states.includes(status)) {
            return [
                new WorkflowContractError(FIELD_OVERLAY_STATUS, `overlay introduces status ${status} not declared in the contract`),
                null,
            ];
        }
    }
    const breadcrumbs = new Map(contract.breadcrumbs);
    for (const [status, body] of overlay.breadcrumbs)
        breadcrumbs.set(status, body);
    ...
    for (const step of overlay.steps) {
        ...
        if (index === undefined) {
            return [
                new WorkflowContractError('<overlay>', `overlay introduces step ${step.id} not declared in the contract`),
                null,
            ];
        }
...
export function buildBreadcrumb(contract, status) {
    ...
    if (body === undefined)
        return [null, GENERIC_BREADCRUMB];
