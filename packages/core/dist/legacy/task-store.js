...
import { clearActiveTask, clearPointersToTask, resolveActiveTask, setActiveTask, } from './active-task.js';
import { gitAddCommit } from './git.js';
import { GATES, PRD_SECTIONS, evaluateCheckLogGate, evaluateFrontendDispatchGate, evaluateStartGate, evaluateStaleAlignmentGate, makeOverride, } from './task-gates.js';
/** 错误消息前缀。 */
...
function normalizeTaskRecord(parsed) {
    const rawHooks = /** @type {Partial<import('./task-store.d.ts').TaskHooks> | undefined} */ (parsed.hooks);
...
const LOG_SEEDS = Object.freeze({
    implement: '{"_example": "implement event log: one JSON object per line; lines without a file field are skipped automatically"}',
    check: '{"_example": "check event log: one JSON object per line; lines without a file field are skipped automatically"}',
});
/**
...
function runHookOnce(root, taskJsonPath, command) {
    return new Promise((resolve) => {
        execFile('/bin/sh', ['-c', command], { cwd: root, env: { ...process.env, [TASK_JSON_ENV]: taskJsonPath } }, (error) => resolve(error ?? null));
    });
}
/**
 * 非阻塞告警统一出口（内部）。
...
function validateParent(root, parentRelPath, childRelPath) {
    ...
    if (parentErr) {
        ...
        throw new Error(`${ERR_PREFIX}: parent task not found: ${parentRelPath}: ${parentErr.message}`);
    }
    if (!parent) {
    ...
    if (parent.status !== TaskStatus.PLANNING && parent.status !== TaskStatus.IN_PROGRESS) {
        throw new Error(`${ERR_PREFIX}: parent task must be planning or in_progress (current: ${parent.status})`);
    }
    return parent;
}
/**
...
function linkChildToParent(root, parentRelPath, childRelPath) {
    const [parentErr, parent] = readTask(root, parentRelPath);
    if (parentErr || !parent) {
        throw new Error(`${ERR_PREFIX}: child task created but parent children not updated: parent task unreadable: ${parentRelPath}`);
    }
    ...
    try {
        ...
        throw new Error(`${ERR_PREFIX}: child task created but parent children not updated: ${toError(error).message}`, { cause: error });
    }
}
/**
...
async function createTaskInternal(root, params) {
    ...
    if (slug.length === 0) {
        throw new Error(`${ERR_PREFIX}: title cannot produce a valid slug: ${JSON.stringify(params.title)}`);
    }
    ...
    const warnings = await runTaskHooks(projectRoot, join(taskDir, FILE_NAMES.taskJson), task.hooks.after_create);
...
async function startTaskInternal(root, params) {
    ...
    if (task.status !== TaskStatus.PLANNING) {
        throw new Error(`${ERR_PREFIX}: only tasks in planning state can be started (current: ${task.status})`);
    }
    if (params.force === true) {
        ...
        const missing = evaluateStartGate(projectRoot, params.taskRelPath, task);
        if (missing.length > 0) {
            throw new Error(`${ERR_PREFIX}: start gate failed: ${missing.join('; ')} ` +
...
function checkTaskInternal(root, params) {
    ...
    if (task.status !== TaskStatus.IN_PROGRESS) {
        throw new Error(`${ERR_PREFIX}: only tasks in in_progress can record a check (current: ${task.status})`);
    }
    ...
    if (forced) {
        ...
        if (missing.length > 0) {
            throw new Error(`${ERR_PREFIX}: check gate failed: ${missing.join('; ')} ` +
                '(pass force: true with a non-empty reason to bypass; the bypass is recorded in task.json overrides)');
        }
    }
    ...
    if (staleMissing.length > 0) {
        if (forced) {
            ...
            throw new Error(`${ERR_PREFIX}: check gate failed: ${staleMissing.join('; ')} ` +
                '(pass force: true with a non-empty reason to bypass; the bypass is recorded in task.json overrides)');
        }
    }
...
function recordAlignmentCredentialInternal(root, taskRelPath, entry) {
    ...
    if (task.status === TaskStatus.COMPLETED) {
        throw new Error(`${ERR_PREFIX}: only planning/in_progress tasks can record an alignment credential ` +
            `(current: ${task.status})`);
    }
    ...
    writeFileAtomic(join(insideWorkloom(projectRoot, taskRelPath), FILE_NAMES.taskJson), serializeTask(stripTaskPath(task)));
    return task;
}
/**
...
function assertTaskForceReason(reason) {
    if (typeof reason !== 'string' || reason.trim() === '') {
        throw new Error(`${ERR_PREFIX}: force: true requires a non-empty reason (the override is recorded in task.json overrides)`);
    }
}
/**
...
function settleExecutorDispatchInternal(root, taskRelPath, entry) {
    ...
    for (let i = task.dispatches.length - 1; i >= 0; i--) {
        ...
        if (record === undefined || record === null || typeof record !== 'object')
            continue;
        if (record.childId !== settle.childId)
            continue;
...
function assertSettleEntry(entry) {
    ...
    if (entry.status !== 'completed' && entry.status !== 'failed') {
        throw new Error(`${ERR_PREFIX}: settle entry status must be completed or failed (got ${String(entry.status)})`);
    }
...
function buildDispatchRecord(entry) {
    ...
    if (!Object.values(EXECUTOR_KINDS).includes(entry.kind)) {
        throw new Error(`${ERR_PREFIX}: invalid dispatch kind: ${String(entry.kind)} (must be one of ${Object.values(EXECUTOR_KINDS).join('/')})`);
    }
    ...
    if (entry.status !== undefined) {
        if (entry.status !== 'running' && entry.status !== 'completed' && entry.status !== 'failed') {
            throw new Error(`${ERR_PREFIX}: invalid dispatch status: ${String(entry.status)} ` +
    ...
    if (entry.modelSource !== undefined && !DISPATCH_MODEL_SOURCE_SET.has(entry.modelSource)) {
        throw new Error(`${ERR_PREFIX}: invalid dispatch modelSource: ${String(entry.modelSource)} ` +
...
export function computeTaskStage(current, kind) {
    if (typeof kind !== 'string' || !Object.values(EXECUTOR_KINDS).includes(kind)) {
        throw new Error(`${ERR_PREFIX}: invalid kind: ${String(kind)} (must be one of ${Object.values(EXECUTOR_KINDS).join('/')})`);
    }
    if (kind === EXECUTOR_KINDS.research)
        return current;
    if (kind === EXECUTOR_KINDS.check)
        return TaskStage.CHECK;
...
async function archiveTaskInternal(root, params) {
    ...
    if (forced) {
        ...
        throw new Error(`${ERR_PREFIX}: archive gate failed: no recorded check in task.json ` +
    ...
    if (staleMissing.length > 0) {
        if (forced) {
            ...
            throw new Error(`${ERR_PREFIX}: archive gate failed: ${staleMissing.join('; ')} ` +
                '(pass force: true with a non-empty reason to bypass; the bypass is recorded in task.json overrides)');
        }
    }
    ...
    if (ptrErr)
        throw ptrErr;
    const warnings = await runTaskHooks(projectRoot, join(archiveDir, FILE_NAMES.taskJson), task.hooks.after_archive);
...
async function autoCommitIfEnabled(root, autoCommit, slug, paths) {
    ...
    if (gitErr)
        return [`git auto-commit failed (archival proceeds anyway): ${gitErr.message}`];
    return [];
}
/**
