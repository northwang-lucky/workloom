...
function collectInternal(root, config) {
    ...
    for (const pkg of packageNames) {
        if (!DIR_NAME_RE.test(pkg))
            continue;
        const layers = listSubdirNames(join(specDir, pkg));
        for (const layer of layers) {
            if (!DIR_NAME_RE.test(layer))
                continue;
            const rel = `${SPEC_REL_PATH}/${pkg}/${layer}/${INDEX_FILE_NAME}`;
            if (fileExists(join(specDir, pkg, layer, INDEX_FILE_NAME)))
                indexes.push(rel);
        }
    }
    ...
    for (const rel of indexes) {
        if (used + Buffer.byteLength(rel) > MAX_GUIDELINES_BYTES)
            break;
