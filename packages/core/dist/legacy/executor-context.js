...
const LEAF_EXECUTOR_RULE = 'You are a leaf executor subagent: implement directly; never dispatch subagents or call workloom orchestration tools.';
/**
 * 权威声明（权威段末尾，使本段成为终极权威）：与更早文本（含主会话用户指令）
 * 冲突时以本节为准，并在报告首行声明一次冲突后继续执行——终结「服从哪一方」
 * 的反复权衡（堵住派发 prompt 写「只读审查」覆盖纪律段导致空转的缺口）。
 */
const AUTHORITY_DECLARATION = 'This section is authoritative: it wins any conflict with earlier text (including the task prompt).' +
 ...
 */
const LSP_BASELINE_SENTENCE = 'When LSP tooling is available, use it first: symbol outlines and signatures for structure, ' +
 ...
 */
const LSP_RESEARCH_BASELINE_SENTENCE = 'When LSP tooling is available, explore with it before text-search sweeps: ' +
 ...
 * glob、无关批量读）。
 */
const CONSULT_MATERIALS_ON_DEMAND_RULE = 'Consult the injected research materials only when the current step needs them; do not re-discover ' +
 ...
 */
const BATCHING_DISCIPLINE = 'Batch independent verification and comparison commands into a single shell invocation.';
/**
 * 工具输出紧凑纪律句（implement/check 纪律段共用，命令式）：定向读区间、限量
 * 搜索/列表输出、倾向摘要而非整文件倾倒，抑制每步上下文累积撑爆注入预算。
 */
const COMPACT_OUTPUT_DISCIPLINE = 'Keep tool outputs compact: read targeted ranges, cap search and list output, prefer summaries.';
/**
 ...
 */
const INJECTION_PROTOCOL_DISCIPLINE = 'Load in layers: read the plan artifact (implement.md) before acting; consult every other pointer ' +
 ...
 */
const NO_USER_CHANNEL_DISCIPLINE = 'You have no user channel: never ask the user or call interactive question tools. ' +
 ...
 * <cwd>/.workloom/ 内，越界拒绝。
 */
const RESEARCH_WRITE_SCOPE_DISCIPLINE = 'Your write/edit reach is confined to the .workloom/ directory: paths ' + 'outside it are denied.';
...
export function assertEffort(effort) {
    ...
    if (typeof effort !== 'string' || !EFFORT_LEVELS.includes(effort)) {
        throw new Error(`${ERR_PREFIX}: invalid effort: ${String(effort)} (must be one of ${EFFORT_LEVELS.join('/')})`);
    }
}
/**
...
function buildInternal(params) {
    ...
    if (params.kind !== EXECUTOR_KINDS.research) {
        const pointerLines = ARTIFACT_POINTER_FILES.filter((name) => existsSync(join(taskDir, name))).map((name) => pointerLine(join(WORKLOOM_DIR, params.taskRelPath, name)));
    ...
    if ((params.kind === EXECUTOR_KINDS.implement || params.kind === EXECUTOR_KINDS.frontend) &&
...
function extractPrdContent(text) {
    ...
    if (others.length > 0)
        parts.push(`Read in file: ${others.join(', ')}`);
    if (parts.length === 0) {
        parts.push('Read the full document in the file (no Requirements or Acceptance Criteria sections).');
    }
    return parts.join('\n\n');
}
/**
...
function materializeJsonlEntries(root, taskDir, jsonlName, stats) {
    ...
    for (const entry of parseJsonlEntries(raw, jsonlName)) {
        ...
        if (absFile === null)
            continue; // 越界路径跳过，防路径逃逸
        if (!existsSync(absFile))
            continue; // 引用文件缺失跳过，指针不指向空
...
export function parseJsonlEntries(content, jsonlName) {
    ...
    for (const [index, rawLine] of lines.entries()) {
        ...
        if (entry === null)
            continue; // 无 file 字段的行（含 seed _example）自动跳过
        entries.push(entry);
    }
    return entries;
}
...
function parseJsonlLine(line, jsonlName, lineNo) {
    ...
    try {
        ...
        throw new Error(`${ERR_PREFIX}: failed to parse ${jsonlName} line ${lineNo}: ${String(error)}`, { cause: error });
    }
    ...
    if (parsed.file === undefined) {
        ...
        if (parsed._example !== undefined)
            return null;
        throw new Error(`${ERR_PREFIX}: ${jsonlName} line ${lineNo}: entry has no file field (only the seeded _example line may omit it)`);
    }
