...
function findLatestJournal(root, developer) {
    ...
    const file = maxNumber > 0 ? `${FILE_NAMES.journalPrefix}${maxNumber}${FILE_NAMES.journalExt}` : null;
...
async function addSessionInternal(root, params) {
    ...
    if (developerReason)
        throw new Error(`${ERR_PREFIX}: ${developerReason}`);
    ...
    if (config.sessionAutoCommit) {
        ...
        if (gitErr) {
            console.warn(`${ERR_PREFIX}: WARNING: git auto-commit failed (journal record proceeds anyway): ${gitErr.message}`);
        }
    }
    return {
...
function listJournalsInternal(root, params) {
    ...
    if (!existsSync(workspaceDir))
        return [];
    const developers = params.developer !== undefined ? [params.developer] : listDeveloperDirs(workspaceDir);
    /** @type {import('./journal.d.ts').JournalSummary[]} */
    const summaries = [];
    for (const developer of developers) {
        const reason = validateDeveloper(developer);
        if (reason)
            throw new Error(`${ERR_PREFIX}: ${reason}`);
...
function listJournalFiles(dir) {
    ...
    for (const name of readdirSync(dir)) {
        const match = JOURNAL_FILE_RE.exec(name);
        if (match)
            files.push({ name, number: Number(match[1]) });
    }
