...
export function loadConfig(root, options = {}) {
    ...
    if (localFile !== null)
        base = evaluateLayerDoc(localFile, base, provenance, 'local');
...
function discoverConfigLayer(dir, layer) {
    ...
    if (existsSync(legacyPath)) {
        throw new WorkloomConfigError(LEGACY_YAML_BY_LAYER[layer], `legacy YAML config is retired; migrate to ${names[0]} or ${names[1]}`);
    }
    const found = names.filter((name) => existsSync(join(dir, name)));
    if (found.length > 1) {
        throw new WorkloomConfigError(layer === 'main' ? 'config' : 'config.local', `ambiguous: both ${found[0]} and ${found[1]} exist; keep exactly one`);
    }
...
function loadJsDoc(file, field) {
    ...
    try {
        ...
        if ( /** @type {NodeJS.ErrnoException} */(error).code === 'ERR_REQUIRE_ESM') {
            throw new WorkloomConfigError(field, `this Node version cannot require() an ES module (${basename(file)}); use CommonJS module.exports, or upgrade Node to >= 22.12 for require(esm)`);
        }
...
function applyGlobalWhitelist(doc) {
    for (const key of Object.keys(doc)) {
        if (GLOBAL_PROJECT_ONLY_FIELDS.has(key)) {
            throw new WorkloomConfigError('global config', `${key} is a project-level field; keep it in the project config, not in $HOME/.workloom`);
        }
        if (!GLOBAL_ALLOWED_TOP_FIELDS.has(key) && key !== 'subagents') {
            throw new WorkloomConfigError('global config', `unsupported global field "${key}" (allowed: ${[
...
function mergeWithDefaults(doc) {
    ...
    if (doc.subagents !== undefined) {
        console.warn('workloom config: "subagents" is deprecated; use "subagent_profiles" instead');
    }
    ...
    if (doc.executor !== undefined) {
        ...
        if (exec.max_concurrent !== undefined) {
            config.executor.maxConcurrent = requireNonNegativeInt('executor.max_concurrent', exec.max_concurrent);
        }
    }
    return config;
}
...
function parsePackages(value) {
    ...
    for (const [name, entry] of Object.entries(map)) {
        ...
        if (spec.type !== undefined)
            parsed.type = requireString(`packages.${name}.type`, spec.type);
        if (spec.git !== undefined)
            parsed.git = requireBoolean(`packages.${name}.git`, spec.git);
        result[name] = parsed;
    }
    return result;
}
    ...
    for (const [name, entry] of Object.entries(map)) {
        ...
        for (const key of Object.keys(spec)) {
            if (key === 'tools' && !allowTools) {
                throw new WorkloomConfigError(`${prefix}.${name}.tools`, 'is only supported under subagent_profiles');
            }
            if (key === 'max_concurrent' && !allowMaxConcurrent) {
                throw new WorkloomConfigError(`${prefix}.${name}.max_concurrent`, 'is only supported under subagent_profiles');
            }
            if (key !== 'model' && key !== 'effort' && key !== 'tools' && key !== 'max_concurrent') {
                throw new WorkloomConfigError(`${prefix}.${name}.${key}`, 
        ...
        if (spec.max_concurrent !== undefined) {
            parsed.maxConcurrent = requireNonNegativeInt(`${prefix}.${name}.max_concurrent`, spec.max_concurrent);
        }
        result[name] = parsed;
    }
    return result;
...
function assertFullProviderModel(field, model) {
    ...
    if (slash <= 0 || slash === model.length - 1) {
        throw new WorkloomConfigError(field, `must be a full "provider/model" identifier: ${model}`);
    }
}
/**
...
function assertNoAmbiguousProfiles(profiles) {
    ...
    if (fallbacks.length > 1) {
        throw new WorkloomConfigError('subagent_profiles', `multiple fallback entries without whenMain (indices ${fallbacks.join(', ')}); only one is allowed`);
    }
    for (let i = 0; i < profiles.length; i++) {
        for (let j = i + 1; j < profiles.length; j++) {
            const overlap = findWhenMainOverlap(profiles[i]?.whenMain, profiles[j]?.whenMain);
            if (overlap !== null) {
                throw new WorkloomConfigError('subagent_profiles', `whenMain conditions overlap between indices ${i} and ${j}: matching value "${overlap.value}" on runtime "${overlap.runtime}"`);
            }
        }
    }
}
...
function findWhenMainOverlap(left, right) {
    ...
    if (leftIsString)
        return findMapOverlapWithString(left, /** @type {Record<string, string>} */ (right));
    if (rightIsString)
        return findMapOverlapWithString(right, /** @type {Record<string, string>} */ (left));
...
export function resolveSubagentDefaults(config, kind, overrides, runtime, mainModel) {
    ...
    if (overrides.model !== undefined) {
        ...
        model = resolveEntryModel(`subagent_profiles[${matched.index}].subagents.${kind}.model`, profileLayer, runtime);
    ...
    return {
        ...
        sources: {
            ...
            effort: overrides.effort !== undefined ? 'param' : effort !== undefined ? 'config' : undefined,
        },
...
function resolveEntryModel(field, entry, runtime) {
    ...
    if (model === undefined || typeof model === 'string')
        return model;
    if (runtime === undefined) {
        throw new WorkloomConfigError(field, 'is a per-runtime map but no runtime was provided');
    }
    const resolved = model[runtime];
    if (resolved === undefined) {
        throw new WorkloomConfigError(field, `missing entry for runtime "${runtime}"`);
    }
    return resolved;
}
/**
...
export function buildConflictNotice(kind, conflicts) {
    return [
        ...
        ...conflicts.map((conflict) => `- ${conflict.field}: config "${conflict.configured}"${configSourceSuffix(conflict)}, passed "${conflict.passed}"`),
...
function configSourceSuffix(conflict) {
    ...
    if (conflict.configuredSource === 'fallback')
        return ' (config: fallback)';
    if (conflict.configuredSource === 'legacy')
...
function requireString(field, value) {
    if (typeof value !== 'string')
        throw new WorkloomConfigError(field, 'must be a string');
    return value;
}
