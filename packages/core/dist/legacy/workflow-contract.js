...
function splitFrontMatter(text) {
    ...
    if (lines[0]?.trim() !== FRONT_MATTER_DELIMITER)
        return null;
    const endIndex = lines.findIndex((line, index) => index > 0 && line.trim() === FRONT_MATTER_DELIMITER);
    if (endIndex === -1) {
...
function parseTagBlocks(bodyText) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (normsOpen !== null) {
            if (normsBlock !== null) {
                throw new WorkflowContractError(FIELD_TAG, `line ${index + 1}: tag blocks must not be nested`);
            }
            if (norms !== null) {
                throw new WorkflowContractError(FIELD_TAG, `line ${index + 1}: norms has multiple tag blocks (ambiguity not allowed)`);
            }
            if (stack.length > 0) {
                throw new WorkflowContractError(FIELD_TAG, `line ${index + 1}: tag blocks must not be nested`);
            }
        ...
        if (normsClose !== null) {
            ...
            for (let j = startLine; j <= index; j += 1)
                masked[j] = TAG_BLOCK_MARKER;
            norms = lines
        ...
        if (open !== null) {
            ...
            if (status === '') {
                throw new WorkflowContractError(FIELD_TAG, `line ${index + 1}: open tag status must not be empty`);
            }
            if (stack.length > 0) {
                throw new WorkflowContractError(FIELD_TAG, `line ${index + 1}: tag blocks must not be nested`);
            }
        ...
        if (close !== null) {
            ...
            if (top === undefined) {
                throw new WorkflowContractError(FIELD_TAG, `line ${index + 1}: tag stack state is inconsistent`);
            }
            if (status !== top.status) {
                throw new WorkflowContractError(FIELD_TAG, `line ${index + 1}: closing tag status ${status} does not match opening tag status ${top.status}`);
            }
            if (breadcrumbs.has(top.status)) {
                throw new WorkflowContractError(FIELD_TAG, `status ${top.status} has multiple tag blocks (ambiguity not allowed)`);
            }
            for (let j = top.startLine; j <= index; j += 1)
                masked[j] = TAG_BLOCK_MARKER;
            breadcrumbs.set(top.status, lines
...
export function parseDocument(markdownText, { requireFrontMatter }) {
    ...
    if (front === null) {
        if (requireFrontMatter) {
            throw new WorkflowContractError(FIELD_FRONT_MATTER, 'document is missing --- delimited front-matter');
        }
    ...
    for (const status of breadcrumbs.keys()) {
        if (!states.includes(status)) {
            throw new WorkflowContractError(FIELD_STATES, `tag block status ${status} is not declared in states`);
        }
    }
    return {
        version,
