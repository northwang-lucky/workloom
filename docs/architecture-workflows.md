...
| W6 研究 | Executor(subagent, agent=research, model/effort 可配) | pi-subagents 派发（registerAgent + 事件总线 request） |
...
- Pi：adapter 经 pi-subagents 事件总线发 `SubagentDelegationRequest`（agent 名 = kind，`thinking` = effort 映射，`model` 原样）；research/implement/check 三个 agent 由 `registerAgent` 运行时注册（front-matter 37 字段，含 tools 白名单与 subagentOnlyExtensions）。
