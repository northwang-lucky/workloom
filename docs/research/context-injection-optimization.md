...
1. **spec 注入降级为索引模式**：jsonl 条目只注入"路径 + reason + 强制先读后判"句；check 角色默认索引模式（其纪律本要求读真实文件）；implement 角色保留内联以保可靠。
...
5. **可靠性护栏**：索引模式附"先读后判"强制句；check 报告须引用实读文件（既有纪律已支持）；implement 默认内联防"子代理不读指针"。
